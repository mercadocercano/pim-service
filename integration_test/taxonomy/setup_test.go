...
// y retorna un httptest.Server con el módulo marketplace-categories configurado.
func newTestServer(t *testing.T) *testServer {
	...
	repo := categoryPersistence.NewMarketplaceCategoryPostgresRepository(db)
	createUC := categoryUsecase.NewCreateMarketplaceCategoryUseCase(repo)
	getAllUC := categoryUsecase.NewGetAllMarketplaceCategoriesUseCase(repo)
	updateUC := categoryUsecase.NewUpdateMarketplaceCategoryUseCase(repo)
	validateUC := categoryUsecase.NewValidateCategoryHierarchyUseCase(repo)

	handler := categoryController.NewMarketplaceCategoryHandler(
		createUC,
		getAllUC,
		updateUC,
		validateUC,
...
// runMigrations ejecuta los archivos SQL de migraciones relevantes en orden.
