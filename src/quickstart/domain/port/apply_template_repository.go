...
type ApplyTemplateRepository interface {
	...
	CreateTenantCategoriesLegacy(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, parentID string) (int, []CreatedCategory, error)

	// CreateTenantCategoriesFromTemplate INSERT categorías desde template JSON
	CreateTenantCategoriesFromTemplate(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, categories []TemplateCategory) (int, []CreatedCategory, error)

	// GetMarketplaceCategoryIDsBySlug SELECT id FROM marketplace_categories
	GetMarketplaceCategoryIDsBySlug(ctx context.Context, exec sharedport.Executor, slugs []string) ([]string, error)

	// CreateTenantBrandsFromGlobalProducts crea marcas desde global_products
	CreateTenantBrandsFromGlobalProducts(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, marketplaceIDs []string, categorySlugs []string, useMarketplaceID bool) (int, []string, error)

	// FindGlobalProduct busca un producto candidato
	FindGlobalProduct(ctx context.Context, exec sharedport.Executor, marketplaceIDs []string, categorySlugs []string, useMarketplaceID bool, brandNames []string) (GlobalProductCandidate, error)

	// EnsureTenantBrand busca o inserta marca
	EnsureTenantBrand(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, brandName string) (string, string, error)

	// ResolveTenantCategory busca categoría por slug (o usa mapas en memoria)
	ResolveTenantCategory(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, tenantCategoriesByMarketplaceID map[string]CreatedCategory, tenantCategoriesBySlug map[string]CreatedCategory, categorySlugByMarketplaceID map[string]string, marketplaceCategoryID string) (*string, *string, error)

	// EnsureTenantProduct busca o inserta producto
	EnsureTenantProduct(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, candidate GlobalProductCandidate, categoryID *string, categoryName *string, brandID string, brandName string) (string, string, string, bool, error)

	// EnsureDefaultVariant inserta variante default
	EnsureDefaultVariant(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, productID string, productName string, productSKU string) (int, error)

	// GlobalProductsHasColumn verifica si global_products tiene una columna
	GlobalProductsHasColumn(ctx context.Context, exec sharedport.Executor, columnName string) bool

	// GlobalProductsHasMarketplaceCategoryID atajo para marketplace_category_id
	GlobalProductsHasMarketplaceCategoryID(ctx context.Context, exec sharedport.Executor) bool
	...
	CreateTenantBrandsFromTemplate(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, brands []TemplateBrand) (int, []string, error)

	// CreateTenantProductsFromTemplate creates all products listed in the template
	CreateTenantProductsFromTemplate(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, products []TemplateProduct, createdCategories []CreatedCategory, createdBrands []string) (int, int, []string, error)

	// CreateTenantAttributesFromTemplate creates attributes and links them to categories
	CreateTenantAttributesFromTemplate(ctx context.Context, exec sharedport.Executor, tenantID uuid.UUID, attributes []TemplateAttribute, createdCategories []CreatedCategory) (int, int, error)
}
