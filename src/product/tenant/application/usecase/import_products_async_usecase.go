...
type FileStorageService interface {
	// Store almacena un archivo y retorna su URL
	Store(ctx context.Context, fileName string, reader io.Reader) (string, error)
	// StoreResults almacena resultados de importación y retorna su URL
	StoreResults(ctx context.Context, jobID string, results interface{}) (string, error)
	// GetReader obtiene un reader para un archivo almacenado
...
func (uc *ImportProductsAsyncUseCase) StartImport(ctx context.Context, req *ImportAsyncRequest) (*ImportAsyncResponse, error) {
	...
	// Guardar trabajo en base de datos
...
func (uc *ImportProductsAsyncUseCase) processImportJob(ctx context.Context, jobID uuid.UUID, fileURL string) {
	...
	if err != nil {
		// No podemos hacer mucho si no encontramos el trabajo
		return
	}
	...
	for i, product := range importResult.ImportedItems {
		...
		if err != nil {
			...
			metrics.RecordValidationError(job.TenantID, "csv_products", "save_error", "product")
		} else {
			successCount++
	...
	// Agregar errores de importación original
	...
	if job.NeedsNotification() {
		if err := uc.notificationSvc.NotifyImportJobComplete(ctx, job); err != nil {
			// Log pero no fallar
...
func (uc *ImportProductsAsyncUseCase) handleJobError(ctx context.Context, job *sharedEntity.ImportJob, err error) {
	...
	metrics.ImportOperationsTotal.WithLabelValues(job.TenantID, "csv_products_async", "failure").Inc()
