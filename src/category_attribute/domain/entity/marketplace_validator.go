...
func (mv *MarketplaceValidator) ValidateCategoryHierarchy(categories []categoryEntity.MarketplaceCategory) error {
	tree, err := categoryEntity.NewCategoryTree(categories)
	if err != nil {
		return err
	}
...
func (mv *MarketplaceValidator) ValidateAttributeType(attrType sharedEntity.AttributeType) error {
	switch attrType {
	case sharedEntity.AttributeTypeText, sharedEntity.AttributeTypeNumber, sharedEntity.AttributeTypeBoolean, sharedEntity.AttributeTypeSelect, sharedEntity.AttributeTypeMultiSelect:
		return nil
	default:
...
func (mv *MarketplaceValidator) ValidateAttributeValue(value string, attrType sharedEntity.AttributeType) error {
	if strings.TrimSpace(value) == "" {
		return errors.New("el valor del atributo no puede estar vacío")
	}

	switch attrType {
	case sharedEntity.AttributeTypeText:
		return mv.validateTextValue(value)
	case sharedEntity.AttributeTypeNumber:
		return mv.validateNumberValue(value)
	case sharedEntity.AttributeTypeBoolean:
		return mv.validateBooleanValue(value)
	case sharedEntity.AttributeTypeSelect, sharedEntity.AttributeTypeMultiSelect:
...
func (mv *MarketplaceValidator) ValidateTenantCategoryMapping(mapping *categoryEntity.TenantCategoryMapping, existingMappings []categoryEntity.TenantCategoryMapping) error {
...
func (mv *MarketplaceValidator) ValidateTenantAttributeExtension(extension *attributeEntity.TenantAttributeExtension, existingExtensions []attributeEntity.TenantAttributeExtension) error {
	// Validar IDs requeridos
...
func (mv *MarketplaceValidator) ValidateBusinessRules(categories []categoryEntity.MarketplaceCategory, attributes []attributeEntity.MarketplaceAttribute) error {
	...
	// Validar que no hay categorías huérfanas (excepto las raíz)
	for _, category := range categories {
		if category.ParentID != nil {
			...
			if !parentExists {
				return errors.New("categoría huérfana detectada: " + category.Name)
			}
		}
	}
...
func (mv *MarketplaceValidator) ValidateMarketplaceConsistency(
	categories []categoryEntity.MarketplaceCategory,
	attributes []attributeEntity.MarketplaceAttribute,
	categoryMappings []categoryEntity.TenantCategoryMapping,
	attributeExtensions []attributeEntity.TenantAttributeExtension,
) error {
