...
import (
	...
	"pim/src/global_catalog/domain/port"
...
func (r *PostgresGlobalProductRepository) scanGlobalProduct(row *sql.Row) (*entity.GlobalProduct, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, sql.ErrNoRows // Devolver el error para que se maneje correctamente arriba
		}
		return nil, fmt.Errorf("error al escanear producto global: %w", err)
	}
