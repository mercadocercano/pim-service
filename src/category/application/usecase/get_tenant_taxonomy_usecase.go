...
import (
	...
	attributePort "pim/src/attribute/domain/port"
	"pim/src/category/application/request"
	"pim/src/category/application/response"
	categoryEntity "pim/src/category/domain/entity"
	categoryPort "pim/src/category/domain/port"
)

// GetTenantTaxonomyUseCase maneja la obtención de la taxonomía completa de un tenant
type GetTenantTaxonomyUseCase struct {
	categoryRepo        categoryPort.MarketplaceCategoryRepository
	mappingRepo         categoryPort.TenantCategoryMappingRepository
	customAttributeRepo attributePort.TenantCustomAttributeRepository
}
...
func (uc *GetTenantTaxonomyUseCase) Execute(
	...
	mappingsByCategory := make(map[string]*categoryEntity.TenantCategoryMapping)
	mappingsByMarketplaceCategory := make(map[string]*categoryEntity.TenantCategoryMapping)
	...
	var marketplaceCategories map[string]*categoryEntity.MarketplaceCategory
	...
	var customAttributes []*attributeEntity.TenantCustomAttribute
...
func (uc *GetTenantTaxonomyUseCase) getMarketplaceCategories(
	ctx context.Context,
	mappings []*categoryEntity.TenantCategoryMapping,
) (map[string]*categoryEntity.MarketplaceCategory, error) {
	categories := make(map[string]*categoryEntity.MarketplaceCategory)
...
func (uc *GetTenantTaxonomyUseCase) buildCategoryNodes(
	...
	mappingsByCategory map[string]*categoryEntity.TenantCategoryMapping,
	marketplaceCategories map[string]*categoryEntity.MarketplaceCategory,
	customAttributes []*attributeEntity.TenantCustomAttribute,
	req *request.GetTenantTaxonomyRequest,
) []response.TenantCategoryNode {
	var nodes []response.TenantCategoryNode

	// Agrupar atributos personalizados por categoría
	attributesByCategory := make(map[string][]*attributeEntity.TenantCustomAttribute)
...
func (uc *GetTenantTaxonomyUseCase) buildCustomAttributeInfos(
	attributes []*attributeEntity.TenantCustomAttribute,
