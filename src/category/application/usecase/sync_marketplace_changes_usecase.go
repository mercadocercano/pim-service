...
func (uc *SyncMarketplaceChangesUseCase) getCategoriesToSync(ctx context.Context, categoryID *string) ([]*categoryEntity.MarketplaceCategory, error) {
	if categoryID != nil {
		category, err := uc.categoryRepo.GetByID(ctx, *categoryID)
		if err != nil {
			return nil, err
		}
		return []*categoryEntity.MarketplaceCategory{category}, nil
	}
...
func (uc *SyncMarketplaceChangesUseCase) syncCategoryForTenant(
	ctx context.Context,
	tenantID string,
	category *categoryEntity.MarketplaceCategory,
	req *request.SyncMarketplaceChangesRequest,
	syncResponse *response.SyncMarketplaceChangesResponse,
) []response.SyncResult {
...
func (uc *SyncMarketplaceChangesUseCase) syncCategoryMapping(
	...
	if err != nil && existingMapping == nil {
		...
		if req.SyncOptions.CreateMissingMappings && !req.DryRun {
			...
			mapping, err := categoryEntity.NewTenantCategoryMapping(tenantID, categoryID, category.ID, nil)
			if err != nil {
