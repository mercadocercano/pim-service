...
func (r BaseListRequest) ToCriteria() Criteria {
	...
	return NewCriteria(
		NewFilters(),
		NewOrder(r.SortBy, sortDir),
		NewPagination(r.Page, r.PageSize),
	)
}
...
func (b *CriteriaBuilder) SetOrder(field, direction string) *CriteriaBuilder {
	if field != "" {
		b.orderField = field
...
func (b *CriteriaBuilder) Build() Criteria {
	...
	if b.orderField != "" {
		...
		order = NewOrder("created_at", "DESC")
	}

	pagination := NewPagination(b.page, b.pageSize)

	return NewCriteria(filters, order, pagination)
}
